  ...
  DataRow getRow(int index, [Color? color]) {
    ...
    return DataRow2.byIndex(
      ...
      cells: [
        ...
        DataCell(Text('${dessert.skill}'),
            onTap: () => _showSnackbar(context,
                'Tapped on a cell with "${dessert.skill}"', Colors.red)),
        DataCell(
          Text(dessert.post),
        ),
...
class ClientsFakeWebService {
  int Function(Client, Client)? _getComparisonFunction(
    ...
    switch (column) {
      ...
      case 'skill':
        return (Client d1, Client d2) => coef * d1.skill.compareTo(d2.skill);
      case 'post':
...
List<Client> _dessertsX3 = _clients.toList()
  ..addAll(_clients.map((i) => Client(
      i.nomer, '${i.skill} x2', '${i.post} x2', '${i.status} x2', i.button)))
  ..addAll(_clients.map((i) => Client(
      i.nomer, '${i.skill} x3', '${i.post} x3', '${i.status} x3', i.button)));
